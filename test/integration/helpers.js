...
helpers._generateCopayersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    ...
    var id45 = Model.Copayer._xPubToCopayerId('polis', xpub_45H.toString());
    ...
    var id44btc = Model.Copayer._xPubToCopayerId('polis', xpub_44H_0H_0H.toString());
...
helpers.toSatoshi = function(polis) {
  if (_.isArray(polis)) {
    return _.map(polis, helpers.toSatoshi);
  } else {
    return Utils.strip(polis * 1e8);
  }
};
...
helpers._parseAmount = function(str) {
  ...
  var re = /^((?:\d+c)|u)?\s*([\d\.]+)\s*(polis|bit|sat)?$/;
