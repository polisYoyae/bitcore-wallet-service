...
describe('Wallet service', function() {
    ...
    describe('shared wallets (BIP44)', function() {
      ...
      it('should create address', function(done) {
        server.createAddress({}, function(err, address) {
          ...
          address.isChange.should.be.false;
          address.coin.should.equal('polis');
  ...
  describe('Multiple request Pub Keys', function() {
    ...
    beforeEach(function() {
      ...
      var copayerId = Model.Copayer._xPubToCopayerId('polis', TestData.copayers[0].xPubKey_44H_0H_0H);
      opts = {
        copayerId: copayerId,
  ...
  describe('#getFeeLevels', function() {
    ...
    it('should get default fees if network cannot be accessed', function(done) {
      ...
      server.getFeeLevels({}, function(err, fees) {
        ...
        var defaults = _.zipObject(_.map(Defaults.FEE_LEVELS['polis'], function(item) {
    ...
    it('should get monotonically decreasing fee values', function(done) {
      _.find(Defaults.FEE_LEVELS['polis'], {
        nbBlocks: 6
  ...
  var addrMap = {
    polis: 'XqHSiRAXd3EmNUPCAqok6ch5XzVWqKg7VD',
  ...
  _.each(['bch','polis'], function(coin) {
      ...
      describe('Tx proposal creation & publishing ' + coin, function() {
        ...
        describe('Fee levels', function() {
          it('should create a tx specifying feeLevel', function(done) {
            //ToDo
            var level = wallet.coin == 'polis' ? 'economy' : 'normal';
            var expected = wallet.coin == 'polis' ? 180e2 : 200e2;
            helpers.stubFeeLevels({
              1: 400e2,
              2: 200e2,
              6: 180e2,
...
//console.log('[server.js.7149]',HugeTxs[1].vin); //TODO
  ...
  describe('#getWalletFromIdentifier', function() {
    ...
    it('should get wallet from tx proposal', function(done) {
      helpers.stubUtxos(server, wallet, '1 polis', function() {
        helpers.stubBroadcast();
        var txOpts = {
          outputs: [{
            toAddress: 'XqHSiRAXd3EmNUPCAqok6ch5XzVWqKg7VD',
  ...
  describe('POLIS & BCH wallets with same seed', function() {
    var server = {},
      wallet = {};
    beforeEach(function(done) {
      helpers.createAndJoinWallet(1, 1, function(s, w) {
    ...
    it('should create address', function(done) {
      server.polis.createAddress({}, function(err, address) {
        should.not.exist(err);
        should.exist(address);
        address.walletId.should.equal(wallet.polis.id);
        address.coin.should.equal('polis');
        address.network.should.equal('livenet');
        address.address.should.equal('Xujpyb3X5oDqfMJEuoXzyfwaSVDSg3vFPm');
        server.bch.createAddress({}, function(err, address) {
          ...
          server.polis.getMainAddresses({}, function(err, addresses) {
            should.not.exist(err);
            addresses.length.should.equal(1);
            addresses[0].coin.should.equal('polis');
            addresses[0].walletId.should.equal(wallet.polis.id);
